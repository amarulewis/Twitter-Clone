...
$("#userSearchTextbox").keydown((event) => {
    ...
    if(value == "" && (event.keycode == 8 || event.keyCode == 8)){
...
function getPostIdFromElement(element){
    ...
    if(postId === undefined) console.log("post id undefined")
...
function createPostHtml(postData, largeFont = false){
    ...
    if(postData.replyTo && postData.replyTo._id){
        ...
        if(!postData.replyTo._id){
            console.log("reply to by is not populated")
...
function outputPosts(results, container){
    ...
    if (results.length == 0){
        container.append("<span class='noResults'>Nothing to show. 3</span>")
    }
}
...
function showMessagePopup(data){
    ...
    var html = createNotificationHtml(data.chat)
    var element = $(html);
    element.hide().prependTo("#notificationList").slideDown("fast");
